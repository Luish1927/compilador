...
class Parser:
    ...
    def _parse_global_declaration(self) -> Union[NodeFunctionDecl, NodeDeclaration]:
        print(f"DEBUG GLOBAL: Token atual é {self._current().type.name}")
    ...
    def _parse_statement(self) -> NodeStmt:
        print(f"DEBUG STATEMENT: Token atual é {self._current().type.name}")
        # Passo 1: Verifica se o statement começa com uma palavra-chave de comando.
        ...
        # Passo 2: Se não for, verifica se é uma declaração de variável (começa com um tipo).
        # ESTA É A VERIFICAÇÃO QUE FOI REMOVIDA INCORRETAMENTE.
        type_tokens = [TokenType.INT, TokenType.FLOAT, TokenType.CHAR, TokenType.STRING, TokenType.BOOL]
        if self._current().type in type_tokens:
            return self._parse_declaration()

        # Passo 3: Se não for NENHUM dos anteriores, SÓ ENTÃO podemos assumir
        # que é uma expressão (como uma atribuição ou chamada de função).
    ...
    def _parse_block(self) -> NodeBlock:
        """Analisa uma lista de statements dentro de um bloco { ... }."""
        statements = []
        
        # Continua analisando statements até encontrar a chave de fechamento '}'
        while not self._current().type == TokenType.RBRACE and not self._is_at_end():
            statements.append(self._parse_statement())
        
        # PASSO CRÍTICO: Consome a chave '}' para finalizar e sair do bloco.
        # Se esta linha faltar, o parser nunca sai do bloco.
    ...
    # parser.py -> _parse_for_statement
    ...
    def _parse_for_statement(self) -> NodeFor:
        ...
        if not self._match(TokenType.SEMI):
            # ANTES: initializer = self._parse_assignment()
            initializer = self._parse_expression() # CORRIGIDO
        ...
        if not self._current().type == TokenType.RPAREN:
            # ANTES: increment = self._parse_assignment()
            increment = self._parse_expression() # CORRIGIDO
        
        self._consume(TokenType.RPAREN, "Esperado ')' após as cláusulas do for")
        
        # Esta parte já deve ter sido corrigida conforme o Ponto 1
        body = self._parse_block()
        
        # O NodeFor pode precisar de ajuste para aceitar NodeExpr em vez de NodeAssignment
        # Em nodes.py: NodeFor.initializer: Optional[NodeExpr]
        # Em nodes.py: NodeFor.increment: Optional[NodeExpr]
        # No seu caso, NodeAssignment é um NodeStmt, não um NodeExpr,
        # então você precisará unificar o tipo ou ajustar o nó.
        # A forma mais limpa é ajustar o NodeFor para aceitar Union[NodeAssignment, NodeFunctionCall] ou simplesmente NodeExpr.
    ...
    def _parse_assignment_stmt(self) -> NodeAssignment:
    ...
    def _parse_function_decl(self) -> NodeFunctionDecl:
        """Analisa uma declaração de função completa."""
        # Consome 'func', nome e parênteses com parâmetros
        ...
        # Garante que o corpo da função comece com '{'
        self._consume(TokenType.LBRACE, "Esperado '{' antes do corpo da função")
        
        # Chama a função para analisar o conteúdo do bloco
        body = self._parse_block()
    ...
    # parser.py -> Adicione este novo método

    def _parse_assignment_expr(self) -> NodeExpr:
        # Analisa a expressão à esquerda do possível '=' (ex: 'result')
        left_expr = self._parse_logical_or()

        # Se o próximo token for um operador de atribuição...
        assign_ops = [TokenType.ASSIGN, TokenType.PLUS_ASSIGN, TokenType.MINUS_ASSIGN, TokenType.STAR_ASSIGN, TokenType.SLASH_ASSIGN]
        if self._current().type in assign_ops:
            op_token = self._advance()

            # O lado direito pode ser outra atribuição (ex: a = b = 5)
            right_expr = self._parse_assignment_expr()

            # O alvo da atribuição deve ser uma variável ou acesso a array
        ...
        return left_expr # Se não for uma atribuição, retorna a expressão da esquerda
